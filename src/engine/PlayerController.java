    ...
    public void control(LevelSelectionContext levelSelectionContext) {
        ...
        switch (gameState) {
            case START_SCREEN_STATE:
                controlStartScreenInput(levelSelectionContext);
                break;
            ...
            case GAME_OVER_STATE:
                controlGameOverInput(levelSelectionContext);
                break;
        }
    }

    private void controlGameOverInput(LevelSelectionContext ctx) {
        ...
        if (input.isKeyPressed(KeyEvent.VK_S)) {
            ...
            ctx.setGameState(GameState.LEVEL_SELECTION_STATE);
        }

        if (input.isKeyPressed(KeyEvent.VK_Q) || input.isKeyPressed(KeyEvent.VK_ESCAPE)) {
            System.exit(0);
        }
    }

    private void controlStartScreenInput(LevelSelectionContext ctx) {
