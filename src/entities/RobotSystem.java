package entities;

import Weapons.MeleeWeapon;
import Weapons.Projectile;
import engine.GameOverState;
import exceptions.*;

import java.util.Iterator;
import java.util.List;

public class RobotSystem {
    Robot robot1;
    Robot robot2;

    private float attackRange;

    public RobotSystem(Robot robot1, Robot robot2) {
        if (robot1 == null || robot2 == null) {
            throw new InvalidRobotStateException("Robots cannot be null");
        }
        this.robot1 = robot1;
        this.robot2 = robot2;
        this.attackRange = 100.0f;
    }

    public void checkAttacksRobots() {
        if (robot1.isAttacking) {

            MeleeWeapon meleeWeapon = robot1.getMeleeWeapon();
            if (meleeWeapon == null) {
                throw new WeaponNotEquippedException("Robot1 has no melee weapon equipped");
            }

            meleeWeapon.use(robot1);

            if (checkIfInAttackRange()) {
                robot2.takeDamage(meleeWeapon.getMeleeDamage());
            }

            robot1.isAttacking = false;
        }

        if (robot2.isAttacking) {

            MeleeWeapon meleeWeapon = robot2.getMeleeWeapon();
            if (meleeWeapon == null) {
                throw new WeaponNotEquippedException("Robot2 has no melee weapon equipped");
            }

            meleeWeapon.use(robot2);

            if (checkIfInAttackRange()) {
                robot1.takeDamage(meleeWeapon.getMeleeDamage());
            }

            robot2.isAttacking = false;
        }
    }

    public void robotsShooting() {
        if (robot1.isShooting) {
            if (robot1.getHandGun() == null) {
                throw new WeaponNotEquippedException("Robot1 has no gun equipped");
            }
            robot1.shoot();
            robot1.isShooting = false;
        }

        if (robot2.isShooting) {
            if (robot2.getHandGun() == null) {
                throw new WeaponNotEquippedException("Robot2 has no gun equipped");
            }
            robot2.shoot();
            robot2.isShooting = false;
        }
    }

    public void checkShootingRobots() {

        if (robot1.getHandGun() == null || robot2.getHandGun() == null) {
            throw new ProjectileSystemException("Gun or projectile system missing");
        }

        List<Projectile> p1 = robot1.getHandGun().getProjectileSystem().getProjectiles();
        List<Projectile> p2 = robot2.getHandGun().getProjectileSystem().getProjectiles();

        if (p1 == null || p2 == null) {
            throw new ProjectileSystemException("Projectile list is null");
        }

        Iterator<Projectile> it = p1.iterator();
        while (it.hasNext()) {
            Projectile p = it.next();

            if (p == null) {
                throw new ProjectileSystemException("Null projectile detected");
            }

            if (p.getHasHitRobot()) {
                robot2.takeDamage(robot1.getHandGun().getBulletDamage());
                it.remove();
            } else if (p.getHasHitTile() || p.getHasReachedBoundary()) {
                it.remove();
            }
        }

        it = p2.iterator();
        while (it.hasNext()) {
            Projectile p = it.next();

            if (p == null) {
                throw new ProjectileSystemException("Null projectile detected");
            }

            if (p.getHasHitRobot()) {
                robot1.takeDamage(robot2.getHandGun().getBulletDamage());
                it.remove();
            } else if (p.getHasHitTile() || p.getHasReachedBoundary()) {
                it.remove();
            }
        }
    }

    public boolean checkIfInAttackRange() {

        if (robot1.getPosition() == null || robot2.getPosition() == null) {
            throw new InvalidPositionException("Robot position is null");
        }

        float disX = Math.abs(robot1.getPosition().getVector2DX() - robot2.getPosition().getVector2DX());
        float disY = Math.abs(robot1.getPosition().getVector2DY() - robot2.getPosition().getVector2DY());

        return (disX * disX + disY * disY) <= (attackRange * attackRange);
    }

    public void checkRespawns() {
        if (robot1.isDead() && robot1.hasLivesRemaining()) {
            if (!robot1.isExploding) robot1.explode();
        }

        if (robot2.isDead() && robot2.hasLivesRemaining()) {
            if (!robot2.isExploding) robot2.explode();
        }
    }

    public GameOverState checkWinCondition() {
        if (!robot1.hasLivesRemaining()) {
            robot1.gameOverforRobo = true;
            if (!robot1.isExploding && !robot1.hasExplodedFinal) robot1.explode();
            return GameOverState.PLAYER2_WINS;
        } else if (!robot2.hasLivesRemaining()) {
            robot2.gameOverforRobo = true;
            if (!robot2.isExploding && !robot2.hasExplodedFinal) robot2.explode();
            return GameOverState.PLAYER1_WINS;
        }
        return GameOverState.NONE;
    }
}