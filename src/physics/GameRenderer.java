...
public class GameRenderer {
    ...
    public void render(Graphics g, Robot robot1, Robot robot2, Level level, Camera camera, String winner, ProjectileSystem ps) {
    ...
    public void renderMenu(Graphics g, int width, int height) {
        ...
        g.drawString("ROBO WARS", width / 2 - 100, height / 2);

        g.setFont(new Font("Arial", Font.PLAIN, 16));
        g.drawString("Press ENTER to continue", width / 2 - 100, height / 2 + 40);
    }

    public void renderLevelSelect(Graphics g, int width, int height, String[] levels, int selected) {
        ...
        g.drawString("Select Level", width / 2 - 80, 100);

        for (int i = 0; i < levels.length; i++) {
            if (i == selected) g.setColor(Color.YELLOW);
            else g.setColor(Color.WHITE);

            g.drawString("Level " + (i + 1), width / 2 - 50, 150 + i * 30);
        }
    }
