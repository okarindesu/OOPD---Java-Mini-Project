...
public class PhysicsSystem {
    ...
    public void update(Robot robot1 , Robot robot2 , Level level) {
        ...
        updateLevel(level) ;
        updateProjectiles(robot1) ;
        updateProjectiles(robot2) ;
    ...
    private void updateProjectiles(Robot robot) {
        List<Projectile> proj = robot.getHandGun().getProjectileSystem().getProjectiles() ;

        for(Projectile p : proj) {
            Vector2D pos = p.getPosition() ;
            Vector2D posAdd = new Vector2D(inputDT * p.getVelocity().getVector2DX() , inputDT * p.getVelocity().getVector2DY()) ;
            pos.addLocal(posAdd) ;
        }
    }
