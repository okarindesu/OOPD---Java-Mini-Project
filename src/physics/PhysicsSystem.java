...
public class PhysicsSystem {
    ...
    private void updateProjectiles(Robot robot) {
        if (robot.getProjectileSystem() != null) {
            robot.getProjectileSystem().update(inputDT);
        }
    }

    public static void updateProjectilePosition(Projectile p, float dt) {
        Vector2D displacement = new Vector2D(p.getVelocity().getVector2DX() * dt, p.getVelocity().getVector2DY() * dt);
